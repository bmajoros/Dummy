...
#   writeScript(self,slurmFile,outFile,jobName,command,moreSBATCH=""):
...
class SlurmWriter:
    ...
    def __init__(self):
        ...
        self.memValue=0
        self.threadsValue=0
        self.exclusiveFlag=None
        self.queue=None

    def exclusive(self):
        self.exclusiveFlag="#SBATCH --exclusive\n"
    ...
    def writeScript(self,slurmFile,outFile,jobName,command,moreSBATCH=""):
        ...
        if(len(self.queue)>0):
            queue="#SBATCH -p "+self.queue+"\n"
        if(self.exclusiveFlag is not None):
            moreSBATCH=self.exclusiveFlag+moreSBATCH
